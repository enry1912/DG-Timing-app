...
export async function handleCallback(request, env) {
  ...
  if (!code || !state || parseCookies(request)[stateCookie] !== state) return Response.redirect(new URL('/?auth=google_failed', env.APP_ORIGIN).toString(), 302);
  ...
  if (!stateRecord) return Response.redirect(new URL('/?auth=google_failed', env.APP_ORIGIN).toString(), 302);
  ...
  if (!tokenResponse.ok) return Response.redirect(new URL('/?auth=google_failed', env.APP_ORIGIN).toString(), 302);
  ...
  if (!infoResponse.ok || !info.sub || !info.email || !info.email_verified) return Response.redirect(new URL('/?auth=google_failed', env.APP_ORIGIN).toString(), 302);
  ...
  return new Response(null, { status: 302, headers: { location: new URL('/?auth=google_success', env.APP_ORIGIN).toString(), 'set-cookie': await createSession(env, userId) } });
    ...
    console.error('Google OAuth callback failed', caught?.message || caught, caught?.stack || '');
    return Response.redirect(new URL('/?auth=google_failed', env.APP_ORIGIN).toString(), 302);
  }
}
