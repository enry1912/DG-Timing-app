import { createAuthToken, hashPassword, verifyPassword } from '../_lib/auth.js';
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const existing = await env.DB.prepare('SELECT users.id, users.email, users.email_verified_at, password_credentials.password_hash FROM users LEFT JOIN password_credentials ON password_credentials.user_id = users.id WHERE users.email = ?1 OR (?2 IS NOT NULL AND users.username = ?2)').bind(normalEmail, normalUsername).first();
    if (existing) {
      if (existing.email !== normalEmail || existing.email_verified_at || !existing.password_hash || !(await verifyPassword(password, existing.password_hash))) return error('That email or username is already registered.', 409);
      const token = await createAuthToken(env, existing.id, 'verify_email', null, 1440);
      ...
      return json({ message: 'A new verification email has been sent.' });
    }
